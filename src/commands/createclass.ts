...
export default class CreateClassCommand extends BaseCommand {
    ...
    async #createChannels(interaction: ChatInputCommandInteraction, className: string, parent: CategoryChannelResolvable): Promise<void> {
        let promises = [
            ...
            interaction.guild!.channels.create({
                ...
                permissionOverwrites: [
                    {
                        id: interaction.guild!.roles.cache.find((role: Role) => role.name === "Estudantes " + className)!.id,
                        allow: [PermissionFlagsBits.ViewChannel],
                        deny: [PermissionFlagsBits.SendMessages]
                    }
                ]
    ...
    async #createRole(interaction: ChatInputCommandInteraction, className: string): Promise<Role> {
    ...
    async #editChannels(guildChannels: Collection<string, GuildBasedChannel>, classRole: Role, classCategory: CategoryChannel): Promise<void> {
        const posts = [
            {
        ...
        for (const channel of guildChannels.values()) {
            if (channel.name === "❓│dúvidas" && channel.parent!.id === classCategory.id && channel.type === ChannelType.GuildForum) {
    ...
    async #givePermissionsForDefaultChannels(classRole: Role, channels: Collection<string, Channel>, faqChannel: TextChannel): Promise<void> {
        for (const channel of channels.values()) {
            ...
            } else if (channel.type === ChannelType.GuildText && channel.id === faqChannel.id) {
                await channel.permissionOverwrites.edit(classRole, {
                    "ReadMessageHistory": true,
                    "SendMessages": false,
                    "ViewChannel": true,
    ...
    async #createInvite(inviteChannel: TextChannel): Promise<Invite> {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const className: string = interaction.options.getString('name')!;
        const faqChannel: TextChannel = interaction.options.getChannel("faq-channel")!;

        // Pega todos os canais e cargos do servidor
        const channels = interaction.guild?.channels.cache as Collection<string, GuildBasedChannel>
        const roles = interaction.guild?.roles.cache as Collection<string, Role>

        // Cria o cargo da turma
        const classRole = await this.#createRole(interaction, className)
       ...
       await this.#givePermissionsForDefaultChannels(classRole, channels, faqChannel)
        ...
        await this.#editChannels(channels, classRole, classCategory)
