...
export class CreateclassCommand implements ICommand {
    build(): SlashCommandOptionsOnlyBuilder {
        return new SlashCommandBuilder()
            ...
            .addStringOption((option) =>
                option.setName('name').setDescription('Nome da turma').setRequired(true),
            )
    ...
    async execute(
        ...
        if (!interaction.guild?.features.includes('COMMUNITY')) {
            await interaction.reply({
                content: '❌ A guilda precisa ser uma comunidade primeiro',
                flags: MessageFlags.Ephemeral,
            });
            return;
        }
        ...
        try {
            ...
            await interaction.editReply({
                content: `Turma ${className} criada com sucesso!\nCargo vinculado: ${result.role}`,
            });
        } catch (error) {
